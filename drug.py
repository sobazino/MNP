...
def C(points):
    ...
    norm_BM = np.linalg.norm(BM)
    dot_products = np.dot(PM, BM)
    norm_PM = np.linalg.norm(PM, axis=1)
    cos_theta = dot_products / (norm_PM * norm_BM)
    ...
    dot_products_shifted = np.dot(PM, BMS)
    norm_BMS = np.linalg.norm(BMS)
    cos_theta_shifted = dot_products_shifted / (norm_PM * norm_BMS)
    angles_radians_shifted = np.arccos(np.clip(cos_theta_shifted, -1.0, 1.0))
    angles_shifted = np.degrees(angles_radians_shifted)
    angles_shifted = np.insert(angles_shifted, index2, 0)
    points = normalize_array(points)
    distances = normalize_array(distances)
    angles = normalize_array(angles)
    angles_shifted = normalize_array(angles_shifted)
    return points, distances, angles, angles_shifted
...
def DRUG(SMILES): 
    ...
    Position, Distances, Angles, AnglesS = C(Position)
    if Position is not None:
        atom_names = np.array(atom_names).reshape(-1, 1)
        Distances = np.array(Distances).reshape(-1, 1)
        Angles = np.array(Angles).reshape(-1, 1)
        AnglesS = np.array(AnglesS).reshape(-1, 1)
        
        atom_features = np.hstack((Position, Distances, Angles, AnglesS, atom_names))
        # atom_features = np.hstack((Position, atom_names))
